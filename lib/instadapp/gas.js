 ...
 * @param {{ authorityAddress?: string }} [options]
 */
async function ensureDsaGas(_signerAddress, _chainId, operation, _estimate = {}, options = {}) {
  if (!isAvocadoEnabled()) {
  ...
  const safeAddress = options.authorityAddress || (await resolveAvocadoSafeAddress());
