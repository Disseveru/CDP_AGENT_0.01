 ...
 * @returns {{ signerAddress?: string, chains?: Record<string, object>, chainId?: number, dsaId?: number, dsaAddress?: string, signerAddress?: string, lastBuildTx?: string }}
 */
...
async function ensureDsaInstance(dsa, web3, signerAddress, options = {}) {
  const chainId = options.chainId ?? dsa.instance?.chainId;
  const chainState = chainId != null ? loadDsaChainState(chainId) : {};
  const requestedId = options.dsaId ?? chainState.dsaId;
