...
async function buildDsaAccount(dsa, web3, options = {}) {
  ...
  const authorityAddress =
    options.authorityAddress || (await resolveDsaAuthorityAddress(options.privateKey));
  const gasFunding = await ensureDsaGas(ownerAddress, chainId, "build");

  if (isAvocadoEnabled()) {
    const { safe } = createAvocadoWallet(options.privateKey);
    const avocadoBuild = await buildDsaAccountViaAvocado(dsa, web3, safe, chainId);
    return {
...
async function ensureDsaInstance(dsa, web3, signerAddress, options = {}) {
  const { resolveDsaAuthorityAddress } = require("./avocadoWallet");
  const chainId = options.chainId ?? resolveDsaChainId();
  ...
  const requestedId =
    options.dsaId ?? resolvePersistedDsaId(state, chainId, ownerAddress);
...
async function castSpells(dsa, web3, spellsInput, options = {}) {
  ...
  if (isAvocadoEnabled()) {
    const { safe } = createAvocadoWallet(options.privateKey);
    return castSpellsViaAvocado(dsa, safe, spellsInput, chainId, {
...
async function buildDsaAccountsForChains(chainIds = DEFAULT_DSA_BUILD_CHAINS, options = {}) {
  const { ensureDsaGas } = require("./gas");
  const { isAvocadoEnabled, resolveDsaAuthorityAddress } = require("./avocadoWallet");
  const { signerAddress, privateKey } = createDsaClient({ chainId: chainIds[0] });
  const authorityAddress = await resolveDsaAuthorityAddress(privateKey);
  const summary = [];
  ...
  for (const chainId of chainIds) {
    ...
    let gasStatus;
    try {
      gasStatus = await ensureDsaGas(signerAddress, chainId, "build", estimate);
    } catch (error) {
      summary.push({
        chainId,
        chain: label,
