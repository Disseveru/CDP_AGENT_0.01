...
test("saveDsaChainState clears other chains when the signer changes", () => {
  const fs = require("node:fs");
  const os = require("node:os");
  const path = require("node:path");
  ...
  saveDsaChainState(8453, { dsaId: 1, dsaAddress: "0xBase" }, oldSigner, dataPath);
  saveDsaChainState(137, { dsaId: 2, dsaAddress: "0xPoly" }, oldSigner, dataPath);
  saveDsaChainState(8453, { dsaId: 3, dsaAddress: "0xNewBase" }, newSigner, dataPath);

  const state = loadDsaState(dataPath);
  assert.equal(state.signerAddress, newSigner);
  assert.deepEqual(Object.keys(state.chains || {}).sort(), ["8453"]);
  assert.equal(state.chains["8453"].dsaId, 3);
  assert.equal(
    resolvePersistedDsaId(state, 137, newSigner),
    undefined,
    "stale polygon dsaId must not survive a signer switch",
  );
